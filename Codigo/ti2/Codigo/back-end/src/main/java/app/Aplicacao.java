        ...
        post("/getuser", (request, response) -> {
        	try {
                ...
                String id = json.get("id").asText();
	         	...
	         	AppUsers user = appUserService.getUserId(Integer.parseInt(id));
	        	if(user!=null) {
		            // Create a JSON object with the data
		            JsonObject userData = new JsonObject();
        ...
        post("/appuser/list", (request, response) -> {
            try {
                ...
                for (AppUsers appUser : appUserList) {
                    ...
                    userJson.addProperty("nome", appUser.getNome());
                    userJson.addProperty("telefone", appUser.getTelefone());
                    userJson.addProperty("biografia", appUser.getBiografia());
                    userJson.addProperty("plataformas", appUser.getPlataformas());
                    userJson.addProperty("jogos", appUser.getJogos());
                    userJson.addProperty("profilePictureUrl", "img/usuario.jpg"); // Adjust this accordingly
